<form action="{{route( 'update_detail_hutang', ['id' => $id] )}}" method="POST" id="formhutang">
@csrf
            <div class="form-group tw-mr-3">
              <label for="tgl_hutang" class="col-form-label">Tgl Hutang:</label>
              <input type="date" class="form-control" id="tgl_hutang" name="tgl_hutang" value="{{$data->tgl_bayar}}">
            </div>
            ...
            <div class="form-group tw-mr-3">
              ...
              <select class="select2 pembayaran_id custom-select" id="pembayaran_id" name="pembayaran_id">
                @foreach($p as $pem)
                  <option value="{{$pem->id}}" @if($data->pembayaran_id == $pem->id) selected @endif>{{$pem->nama_bayar}}</option>
                @endforeach
              </select>
            </div>

            <div class="form-group tw-mr-3" id="input_giro" @if($data->Pembayaran->nama_bayar == "Cash") hidden="true" @endif>
